...
async function sendCode(email) {
  ...
  if (!r.ok) throw new Error(j?.message || "Kon code niet versturen");
  return j;
}
...
async function verifyCode(email, code) {
  ...
  return j; // veronderstelt dat server een httpOnly sessie-cookie zet
}
...
export default function LoginPage() {
  ...
  // redirect comes from ?redirect=/pad OR from location.state?.from?.pathname
  const qs = new URLSearchParams(location.search);
  const redirect =
    qs.get("redirect") || location.state?.from?.pathname || "/dashboard";
  ...
  useEffect(() => {
    getMe()
      .then((me) => {
        if (me?.user) nav(redirect, { replace: true });
      })
      .catch(() => {});
  }, [nav, redirect]);

  // teardown cooldown interval
  useEffect(() => {
  ...
  async function onSend(e) {
    ...
    try {
      ...
      setStep(STEP.OTP);
      setNotice("We hebben je een verificatiecode gemaild.");
  ...
  async function onVerify(e) {
    ...
    setLoading(true);
    try {
      await verifyCode(email.trim(), clean);
      ...
      <section className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg ring-1 ring-black/5">
        <header className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Veilig & wachtwoordloos inloggen met een e-mailcode.
          </p>
        </header>
